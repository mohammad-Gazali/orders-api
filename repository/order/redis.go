...
func (r *RedisRepo) Insert(ctx context.Context, order models.Order) error {
	...
	// this is a transaction for handling adding both order and its id to "orders" set
	// we don't want to add order and failing by adding its id to the "orders" set and vice versa
	// so we defined this transaction to make satisfying of the both process is required and for each process it can't be done without the other
	txn := r.Client.TxPipeline()

	res := txn.SetNX(ctx, key, string(data), 0)
	if err := res.Err(); err != nil {
		txn.Discard() // this method is used for discard other changes to this transaction pipeline
...
func (r *RedisRepo) UpdateByID(ctx context.Context, order models.Order) error {
	data, err := json.Marshal(order)
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	//! there is a strange issue with redis sscan command, it returns all the results event the count param
